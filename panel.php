...
if ($method === 'POST' && !$locked) {
    ...
    if (panel_session_active()) {
        if ($action === 'dev_on') {
            gate_dev_set(true);
            $flash = 'Dev mode ACTIVADO. Puedes navegar /simulador/ libremente por 8h.';
            $flash_type = 'success';
        }
        if ($action === 'dev_off') {
            gate_dev_set(false);
...
<?php else: ?>
    <!-- ============ DASHBOARD ============ -->
    <h1>⚡ Control Panel</h1>
    <p class="sub">Activa Dev Mode para acceder a <code>/simulador/</code> sin pasar por el anuncio.</p>

    <div class="status">
      <div class="dot <?= $dev_on ? 'on' : 'off' ?>"></div>
      <div class="status-text">
        <strong>Dev Mode: <?= $dev_on ? 'ACTIVO' : 'Inactivo' ?></strong>
        <small><?= $dev_on ? 'Cookie _dev emitida (8h). Bypass de gate activado.' : 'El gate está bloqueando todo acceso normal a /simulador/.' ?></small>
      </div>
    </div>
...
<?php if ($dev_on): ?>
    <div class="row">
      <a class="btn btn-ghost" href="/simulador/index.php" target="_blank">🌐 Abrir /simulador/</a>
